capture cd "D:\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Jalisco - 1995, 1997, 2000, 2003, 2006, 2009, 2012"
capture cd "C:\Users\Horacio\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Jalisco - 1995, 1997, 2000, 2003, 2006, 2009, 2012"

*************************************************************************************************
*************************************************************************************************
*************************************************************************************************

import excel Ayu_Seccion_1997_No_LN.xlsx, clear firstrow case(lower)
...
replace uniqueid=14023 if municipality =="CIUDAD GUZMAN"
...
replace uniqueid=14027 if municipality =="CUAUTITLAN DE GARCIA BARRAGAI" | municipality =="CUAUTITLAN"
...
replace uniqueid=14044 if municipality =="IXTLAHUACAN DE LOS MEMBRILLO" | municipality =="IXTLAHUACAN DE LOS MEMBRILLOS"
...
replace uniqueid=14117 if municipality =="VILLA OBREGON"
...
foreach var in PAN PRI PRD PartCardenista PT PVEM PPS PD total valid {
...
replace listanominal = . if missing>=1
drop missing 
...
egen valid = rowtotal(PAN PRI PPS PRD PartCardenista PARM PD PT PVEM PPJ)

foreach var in PAN PRI PPS PRD PartCardenista PARM PD PT PVEM PPJ total valid {
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PartCardenista inv_mun_PT inv_mun_PVEM inv_mun_PPS inv_mun_PD inv_mun_PARM inv_mun_PPJ, gen(PAN_r PRI_r PRD_r PartCardenista_r PT_r PVEM_r PPS_r PD_r PARM_r PPJ_r)
drop inv_mun_*
...
replace municipality = "Teocuitatlan de Corona" if uniqueid ==14092 & section== 3338

drop turnout listanominal mun_turnou
...
replace municipality = "Tonala" if uniqueid ==14101 & section== 3311
...
egen valid = rowtotal(PAN PRI PRD PT PVEM PC PSN PAS ElBarzon MexicoPosible PLM FC PRD_PAS PRD_PC PRD_PVEM PVEM_PAS PRD_PVEM_PAS)

foreach var in PAN PRI PRD PT PVEM PC PSN PAS ElBarzon MexicoPosible PLM FC PRD_PAS PRD_PC PRD_PVEM PVEM_PAS PRD_PVEM_PAS total valid {
bys uniqueid: egen mun_`var'= sum(`var') 
gen inv_mun_`var'= 1/mun_`var'
}

* Drop duplicated section number
drop if section==2484
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PT inv_mun_PVEM inv_mun_PC inv_mun_PSN inv_mun_PAS inv_mun_ElBarzon inv_mun_MexicoPosible inv_mun_PLM inv_mun_FC inv_mun_PRD_PAS inv_mun_PRD_PC inv_mun_PRD_PVEM inv_mun_PVEM_PAS inv_mun_PRD_PVEM_PAS, ///
	gen(PAN_r PRI_r PRD_r PT_r PVEM_r PC_r PSN_r PAS_r ElBarzon_r MexicoPosible_r PLM_r FC_r PRD_PAS_r PRD_PC_r PRD_PVEM_r PVEM_PAS_r PRD_PVEM_PAS_r)
drop inv_mun_*
...
import excel Ayu_Seccion_2006_No_LN.xlsx, clear firstrow case(lower)
...
replace uniqueid=14017 if municipality =="AYUTLA"
replace uniqueid=14019 if municipality =="BOLANOS"
replace uniqueid=14020 if municipality =="CABO CORRIENTES" | municipality=="CABO CORRIENTE"
replace uniqueid=14117 if municipality =="CANADAS DE OBREGON"
replace uniqueid=14021 if municipality =="CASIMIRO CASTILLO"
replace uniqueid=14030 if municipality =="CHAPALA"
replace uniqueid=14031 if municipality =="CHIMALTITAN"
...
replace uniqueid=14025 if municipality =="COLOTLAN"
replace uniqueid=14026 if municipality =="CONCEPCION DE BUENOS AIRES"
replace uniqueid=14027 if municipality =="CUAUTITLAN DE GARCIA BARRAGAI" | municipality =="CUAUTITLAN" | municipality=="CUAUTITLAN DE GARCIA BARRAGAN"
replace uniqueid=14028 if municipality =="CUAUTLA"
replace uniqueid=14029 if municipality =="CUQUIO"
replace uniqueid=14033 if municipality =="DEGOLLADO"
replace uniqueid=14034 if municipality =="EJUTLA"
...
replace uniqueid=14018 if municipality =="LA BARCA"
replace uniqueid=14043 if municipality =="LA HUERTA"
replace uniqueid=14057 if municipality =="MANZANILLA DE LA PAZ" | municipality=="LA MANZANILLA DE LA PAZ"
replace uniqueid=14053 if municipality =="LAGOS DE MORENO"
replace uniqueid=14055 if municipality =="MAGDALENA"
replace uniqueid=14056 if municipality =="MANUEL M.DIEGUEZ"
replace uniqueid=14058 if municipality =="MASCOTA"
...
import excel Ayu_Seccion_2009_No_LN.xlsx, clear firstrow case(lower)
...
* Drop duplicated sections where there are zero votes
drop if section==995 & muni=="SAN PEDRO TLAQUEPAQUE"
drop if section==2729 & muni=="EL SALTO"
drop if section==2486 & muni=="GUADALAJARA"
...
replace uniqueid=14096 if municipality =="TIZAPAN EL ALTO"
replace uniqueid=14097 if municipality =="TLAJOMULCO DE ZUNIGA"
replace uniqueid=14098 if municipality =="TLAQUEPAQUE" | municipality =="SAN PEDRO TLAQUEPAQUE"
replace uniqueid=14099 if municipality =="TOLIMAN"
replace uniqueid=14100 if municipality =="TOMATLAN"
replace uniqueid=14101 if municipality =="TONALA"
replace uniqueid=14102 if municipality =="TONAYA"
...
import excel "Juchitlan Extraordinario 1998.xlsx", sheet("Sheet1") firstrow clear
...
collapse (sum) PAN - PRI total, by (municipality section)

g uniqueid = 14052
replace municipality = "JUCHITLAN EXTRAORDINARIO"

egen valid = rowtotal(PAN PRI)

preserve
use "..\..\all_months_years.dta", clear
keep if month==12 & year==1997 & ed==14
rename seccion section
rename lista listanominal
...
foreach var in PAN PRI total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
	gen inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI, gen(PAN_r PRI_r)
drop inv_mun_*
...
save "Jalisco_Section_1998 Extraordinario.dta", replace

*************************************************************************************************
*************************************************************************************************
*************************************************************************************************

import excel "Tamazula Extraordinario 2004.xlsx", sheet("Sheet1") firstrow clear
...
collapse (sum) PAN - MC total, by (municipality section)

g uniqueid = 14085
replace municipality = "TAMAZULA EXTRAORDINARIO"

egen valid = rowtotal(PAN PRI_PVEM PRD PT MC)

preserve
use "..\..\all_months_years.dta", clear
keep if month==9 & year==2003 & ed==14
rename seccion section
rename lista listanominal
save "merge.dta", replace
restore
...
foreach var in PAN PRI_PVEM PRD PT MC total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
	gen inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI_PVEM inv_mun_PRD inv_mun_PT inv_mun_MC, gen(PAN_r PRI_PVEM_r PRD_r PT_r MC_r)
drop inv_mun_*
...
save "Jalisco_Section_2004 Extraordinario.dta", replace

*************************************************************************************************
*************************************************************************************************
*************************************************************************************************

import excel "Tuxcueca Extraordinario 2007.xlsx", sheet("Sheet1") firstrow clear

rename Sección section
drop if section==.

rename Totales total
rename PVEM_CONV PVEM_MC

drop if total==. | total==0

collapse (sum) PAN - PVEM_MC total, by (municipality section)

g uniqueid = 14107
replace municipality = "TUXCUECA EXTRAORDINARIO"

egen valid = rowtotal(PAN PRI PT ASC PVEM_MC)

preserve
use "..\..\all_months_years.dta", clear
keep if month==1 & year==2007 & ed==14
rename seccion section
rename lista listanominal
save "merge.dta", replace
restore
...
foreach var in PAN PRI PT ASC PVEM_MC total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
	gen inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PT inv_mun_ASC inv_mun_PVEM_MC, gen(PAN_r PRI_r PT_r ASC_r PVEM_MC_r)
drop inv_mun_*

gen winner = "PAN" if PAN_r==1  
replace winner = "PRI" if PRI_r==1
replace winner = "PT" if PT_r==1
replace winner = "ASC" if ASC_r==1
replace winner = "PVEM_MC" if PVEM_MC_r==1
drop *_r
...
save "Jalisco_Section_2007 Extraordinario.dta", replace

*************************************************************************************************
*************************************************************************************************
*************************************************************************************************

import excel "CASxCAS_GomezF2009.xls", sheet("Gomez farias") cellrange(A7:N25) firstrow clear
save "append.dta", replace

import excel "CASxCAS_SnCristobal2009.xls", sheet("San cristobal") cellrange(A7:N19) firstrow clear
...
rename Municipio municipality
rename VotaciónTotalEmitida total
rename PRD_PT_CONV PRD_PT_MC

drop if total==. | total==0

collapse (sum) PAN - PRD_PT_MC total, by (municipality section)

g uniqueid= 0
replace uniqueid=14079 if municipality =="GÓMEZ FARÍAS"
replace uniqueid=14071 if municipality =="SAN CRISTÓBAL DE LA BARRANCA"

replace municipality = "GÓMEZ FARÍAS EXTRAORDINARIO" if municipality =="GÓMEZ FARÍAS"
replace municipality = "SAN CRISTÓBAL DE LA BARRANCA EXTRAORDINARIO" if municipality =="SAN CRISTÓBAL DE LA BARRANCA"

egen valid = rowtotal(PAN PRI PVEM PANAL PRD_PT_MC)

preserve
use "..\..\all_months_years.dta", clear
keep if month==11 & year==2009 & ed==14
rename seccion section
rename lista listanominal
save "merge.dta", replace
restore
...
foreach var in PAN PRI PVEM PANAL PRD_PT_MC total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
	gen inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PVEM inv_mun_PANAL inv_mun_PRD_PT_MC, gen(PAN_r PRI_r PVEM_r PANAL_r PRD_PT_MC_r)
drop inv_mun_*
...
replace winner = "PVEM" if PVEM_r==1
replace winner = "PANAL" if PANAL_r==1
replace winner = "PRD_PT_MC" if PRD_PT_MC_r==1
drop *_r
...
save "Jalisco_Section_2009 Extraordinario.dta", replace
...
append using "Jalisco_Section_1998 Extraordinario.dta"
append using Jalisco_Section_2000.dta
append using Jalisco_Section_2003.dta
append using "Jalisco_Section_2004 Extraordinario.dta"
append using Jalisco_Section_2006.dta
append using "Jalisco_Section_2007 Extraordinario.dta"
append using Jalisco_Section_2009.dta
append using "Jalisco_Section_2009 Extraordinario.dta"
append using Jalisco_Section_2012.dta

erase Jalisco_Section_1995.dta
erase Jalisco_Section_1997.dta
erase "Jalisco_Section_1998 Extraordinario.dta"
erase Jalisco_Section_2000.dta
erase Jalisco_Section_2003.dta
erase "Jalisco_Section_2004 Extraordinario.dta"
erase Jalisco_Section_2006.dta
erase "Jalisco_Section_2007 Extraordinario.dta"
erase Jalisco_Section_2009.dta
erase "Jalisco_Section_2009 Extraordinario.dta"
...
foreach var in PRI PRD PT PC PVEM PANAL PD PSD PAS PartCardenista {
	g `var'_winner =0
...
saveold "..\Jalisco_ALL.dta", replace version(12)
