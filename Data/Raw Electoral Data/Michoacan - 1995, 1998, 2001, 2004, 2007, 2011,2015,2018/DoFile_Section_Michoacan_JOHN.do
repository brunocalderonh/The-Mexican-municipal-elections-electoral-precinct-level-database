capture cd "C:\Users\jmarshall\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Michoacan - 1995, 1998, 2001, 2004, 2007, 2011"
...
replace uniqueid=16011 if municipality =="BRISE�AS" | strpos(municipality, "BRISE")>0
...
egen valid = rowtotal(PAN PRI PRD PartCardenista PT)

foreach var in PAN PRI PRD PartCardenista PT total valid{
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PartCardenista inv_mun_PT, gen(PAN_r PRI_r PRD_r PartCardenista_r PT_r)
drop inv_mun_*
...
collapse (sum) missing pan - total listanominal, by(municipality section)
...
rename uniqueid orden
capture merge m:1 orden year using "..\..\..\Final Data\incumbents.dta", keepusing(winner_mun)
drop if _merge==2
drop _merge
rename winner_mun winner
rename orden uniqueid
...
gen winner = "PAN" if PAN_r==1  
replace winner = "PRI" if PRI_r ==1
replace winner = "PRD_PT_PC_PVEM_PAS_PSN" if PRD_PT_PC_PVEM_PAS_PSN_r==1
drop *_r
...
import excel "resultados_electorales_2005.xls", sheet("Ayuntamiento, por Casilla, 2005") cellrange(A7:K36) firstrow clear 

keep if strpos(B, "ecci")>0

g section = substr(B, -4, 4)
drop A B
rename ListaNominal listanominal
rename VotosTotales total

g municipality = "TUMBISCATIO EXTRAORDINARIO"
g uniqueid = 16096
...
collapse (sum) listanominal-PC total, by (uniqueid municipality section)

gen turnout = total/listanominal

egen valid = rowtotal(PRI PRD PT PC)

foreach var in PRI PRD PT PC total listanominal valid {
	bys uniqueid: egen mun_`var'= sum(`var') 
	gen inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PRI inv_mun_PRD inv_mun_PT inv_mun_PC, gen(PRI_r PRD_r PT_r PC_r)
drop inv_mun_*

gen winner = "PRI" if PRI_r ==1
replace winner = "PRD" if PRD_r==1
replace winner = "PT" if PT_r==1
replace winner = "PC" if PC_r==1
drop *_r
...
save Michoacan_Section_2005.dta, replace

*************************************************************************************************
*************************************************************************************************
*************************************************************************************************

import excel Ayu_Seccion_2007.xlsx, clear firstrow case(lower)
...
replace winner = "PAN_PRI" if PAN_PRI_r==1  
replace winner = "PAN_PRI_PVEM" if PAN_PRI_PVEM_r==1  
replace winner = "PAN_PVEM" if PAN_PVEM_r ==1
replace winner = "PAN_PVEM_PANAL" if PAN_PVEM_PANAL_r ==1
...
import excel "resultados_2008.xls", sheet("Ayuntamiento Yurécuaro ") cellrange(A8:L46) firstrow clear 

rename Sección section
rename K total

g municipality = "Yurécuaro EXTRAORDINARIO"
g uniqueid = 16106

drop if total==. | total==0 

collapse (sum) PAN-PT total, by (uniqueid municipality section)

preserve
use "..\..\all_months_years.dta", clear
keep if ed==16 & month==4 & year==2008
...
rowranks inv_mun_PRI inv_mun_PRD inv_mun_PT inv_mun_PAN, gen(PRI_r PRD_r PT_r PAN_r)
drop inv_mun_*

gen winner = "PRI" if PRI_r ==1
replace winner = "PRD" if PRD_r==1
replace winner = "PT" if PT_r==1
replace winner = "PAN" if PAN_r==1
drop *_r

gen year = 2008
gen month ="May"

save Michoacan_Section_2008.dta, replace
...
replace winner = "PANAL" if PANAL_r==1  
replace winner = "PAN_PRI_PVEM" if PAN_PRI_PVEM_r==1  
replace winner = "PAN_PRI_PVEM_PANAL" if PAN_PRI_PVEM_PANAL_r==1  
replace winner = "PAN_PRI_PANAL" if PAN_PRI_PANAL_r==1  
replace winner = "PAN_PANAL" if PAN_PANAL_r==1  
replace winner = "PRI_PRD_PVEM_PC_PANAL" if PRI_PRD_PVEM_PC_PANAL_r==1
replace winner = "PRI_PVEM" if PRI_PVEM_r==1
replace winner = "PRD_PT" if PRD_PT_r==1
replace winner = "PRD_PT_PC" if PRD_PT_PC_r==1
replace winner = "PRD_PC" if PRD_PC_r==1
...
import excel "Resultados_electorales_extraordinaria_2012.xlsx", sheet("morelia2012_bd") cellrange(A8:S904) firstrow clear
...
g municipality = "MORELIA EXTRAORDINARIO"
g uniqueid = 16053
...
collapse (sum) PAN_PANAL PRD_PT_PMC PRI_PVEM total, by (uniqueid municipality section)
...
egen valid = rowtotal(PAN_PANAL PRD_PT_PMC PRI_PVEM)

foreach var in PAN_PANAL PRD_PT_PMC PRI_PVEM total listanominal valid {
	bys uniqueid: egen mun_`var'= sum(`var') 
	gen inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN_PANAL inv_mun_PRD_PT_PMC inv_mun_PRI_PVEM, gen(PAN_PANAL_r PRD_PT_PMC_r PRI_PVEM_r)
drop inv_mun_*

gen winner = "PRI_PVEM" if PRI_PVEM_r ==1
replace winner = "PRD_PT_PMC" if PRD_PT_PMC_r==1
replace winner = "PAN_PANAL" if PAN_PANAL_r==1
drop *_r
...
save Michoacan_Section_2012.dta, replace
...
append using Michoacan_Section_2005.dta
append using Michoacan_Section_2007.dta
append using Michoacan_Section_2008.dta
append using Michoacan_Section_2011.dta
append using Michoacan_Section_2012.dta
...
erase Michoacan_Section_2005.dta
erase Michoacan_Section_2007.dta
erase Michoacan_Section_2008.dta
erase Michoacan_Section_2011.dta
erase Michoacan_Section_2012.dta
...
saveold "Michoacan_ALL.dta", replace version(12)
