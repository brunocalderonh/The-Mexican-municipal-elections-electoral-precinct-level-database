...
import excel "Extraordinario 2005.xlsx", sheet("Sheet1") firstrow clear

destring CASILLA, force g(section)
replace section = 2220 if _n>=2

egen total = rowtotal(PAN PRI PT NR NULO)

collapse (sum) PAN PRI PT total, by (municipality section)

g uniqueid = 30096

preserve
use "..\..\all_months_years.dta", clear
keep if ed==30 & month==6 & year==2005
...
egen valid = rowtotal(PAN PRI PT)

foreach var in PAN PRI PT listanominal total valid {
	bys uniqueid: egen mun_`var'= sum(`var') 
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PT, gen(PAN_r PRI_r PT_r)
drop inv_mun_*
...
save Veracruz_Section_2005.dta, replace
...
replace uniqueid=30063 if municipality =="CHONTLA"
replace uniqueid=30064 if municipality =="CHUMATLAN"
replace uniqueid=30035 if municipality =="CITLALTÉPETL" | strpos(municipality, "CITLAL")>0
replace uniqueid=30036 if municipality =="COACOATZINTLA"
replace uniqueid=30037 if municipality =="COAHUITLAN"
replace uniqueid=30038 if municipality =="COATEPEC"
replace uniqueid=30039 if municipality =="COATZACOALCOS"
...
replace uniqueid=30077 if municipality =="ISLA"
replace uniqueid=30078 if municipality =="IXCATEPEC"
replace uniqueid=30079 if municipality =="IXHUACÁN DE LOS REYES" | strpos(municipality, "DE LOS REYES")>0 & strpos(municipality, "IXHUA")>0
replace uniqueid=30083 if municipality =="IXHUATLAN DE MADERO"
...
replace uniqueid=30125 if municipality =="PASO DEL MACHO"
replace uniqueid=30128 if municipality =="PEROTE"
replace uniqueid=30129 if municipality =="PLATÓN SÁNCHEZ" | (strpos(municipality, "PLAT")>0 & strpos(municipality, "NCHEZ")>0)
replace uniqueid=30130 if municipality =="PLAYA VICENTE"
...
gen winner = "PAN" if PAN_r==1  
replace winner = "PRI_PVEM_PRV" if PRI_PVEM_PRV_r ==1 
...
* Missing data: special election in 2011
...
foreach var in PAN PRI_PVEM_PANAL PRD PT PC AVE PartCardenista total valid{
bys uniqueid: egen mun_`var'= sum(`var') 
gen inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI_PVEM_PANAL inv_mun_PRD inv_mun_PT inv_mun_PC inv_mun_AVE inv_mun_PartCardenista, gen(PAN_r PRI_PVEM_PANAL_r PRD_r PT_r PC_r AVE_r PartCardenista_r)
drop inv_mun_*
...
* Missing data: special elections in 2014
...
append using  Veracruz_Section_2005.dta
...
erase Veracruz_Section_2005.dta
...
foreach var in PRI PRD PT PCP PVEM PANAL PD PSD PAS PSN PartidoMexicoPosible CDPPN PUP PEC PFC PAC PJS PST PRV AVE  PartCardenista PARM {
gen `var'_winner =0
...
saveold Veracruz_ALL.dta, replace version(12)
