capture cd "D:\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Durango - 1995, 1998, 2001, 2004, 2007, 2010, 2013"
...
egen valid = rowtotal(PAN PRI PRD PartCardenista PT PVEM)

foreach var in PAN PRI PRD PartCardenista PT PVEM total valid{
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PartCardenista inv_mun_PT inv_mun_PVEM, gen(PAN_r PRI_r PRD_r PartCardenista_r PT_r PVEM_r)
drop inv_mun_*
...
replace uniqueid=10005 if municipality =="DURANGO"
replace uniqueid=10018 if municipality =="EL ORO"
replace uniqueid=10006 if strpos(municipality, "BOLIVAR")>0
replace uniqueid=10007 if strpos(municipality, "MEZ PALACIO")>0
replace uniqueid=10008 if municipality =="GUADALUPE  VICTORIA"
replace uniqueid=10009 if strpos(municipality, "GUANACEV")>0
replace uniqueid=10010 if municipality =="HIDALGO"
replace uniqueid=10011 if municipality =="INDE"
replace uniqueid=10012 if municipality =="LERDO"
replace uniqueid=10013 if municipality =="MAPIMI"
...
replace uniqueid=10017 if municipality =="OCAMPO"
replace uniqueid=10019 if municipality =="OTAEZ"
replace uniqueid=10020 if strpos(municipality, "NUCO DE CORONADO")>0
replace uniqueid=10021 if strpos(municipality, "ON BLANCO")>0
replace uniqueid=10022 if municipality =="POANAS"
replace uniqueid=10023 if municipality =="PUEBLO NUEVO"
replace uniqueid=10024 if municipality =="RODEO"
replace uniqueid=10025 if municipality =="SAN BERNARDO"
...
replace uniqueid=10031 if municipality =="SANTA CLARA"
replace uniqueid=10032 if municipality =="SANTIAGO PAPASQUIARO"
replace uniqueid=10033 if strpos(municipality, "CHIL")>0
replace uniqueid=10034 if municipality =="TAMAZULA"
replace uniqueid=10035 if municipality =="TEPEHUANES"
replace uniqueid=10036 if municipality =="TLAHUALILO"
replace uniqueid=10037 if municipality =="TOPIA"
...
replace winner = "PRI_PANAL_PD" if PRI_PANAL_PD_r ==1
replace winner = "PRD" if PRD_r==1
replace winner = "PVEM" if PVEM_r==1
...
foreach var in PAN_PRD_PC PRI_PVEM_PANAL_PD PT total valid listanominal{
bys uniqueid: egen mun_`var'= sum(`var') 
gen inv_mun_`var'= 1/mun_`var'
}
...
gen winner = "PAN_PRD_PC" if PAN_PRD_PC_r==1  
replace winner = "PRI_PVEM_PANAL_PD" if PRI_PVEM_PANAL_PD_r ==1
replace winner = "PT" if PT_r==1
drop *_r
...
import delimited Ayu_Seccion_2013.csv, clear
...
saveold Durango_ALL.dta, replace version(12)
