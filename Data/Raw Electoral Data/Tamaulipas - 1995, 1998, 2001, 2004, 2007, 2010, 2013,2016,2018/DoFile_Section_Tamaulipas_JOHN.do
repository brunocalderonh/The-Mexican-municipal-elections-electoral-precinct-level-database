capture cd "D:\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Tamaulipas - 1995, 1998, 2001, 2004, 2007, 2010, 2013"
capture cd "C:\Users\Horacio\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Tamaulipas - 1995, 1998, 2001, 2004, 2007, 2010, 2013"
...
egen valid = rowtotal(PAN PRI PRD PT PVEM PARM PartCardenista)

foreach var in PAN PRI PRD PT PVEM PARM PartCardenista total valid{
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PT inv_mun_PVEM inv_mun_PARM inv_mun_PartCardenista, gen(PAN_r PRI_r PRD_r PT_r PVEM_r PARM_r PartCardenista_r)
drop inv_mun_*
...
egen valid = rowtotal(PAN PRI PRD PartCardenista PARM PT PVEM)

foreach var in PAN PRI PRD PartCardenista PARM PT PVEM total valid{
bys uniqueid: egen mun_`var'= sum(`var') 
gen inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PartCardenista inv_mun_PARM inv_mun_PT inv_mun_PVEM, gen(PAN_r PRI_r PRD_r PartCardenista_r PARM_r PT_r PVEM_r)
drop inv_mun_*
...
replace uniqueid=28032 if municipality =="REYNOSA"
replace uniqueid=28033 if municipality =="R�O BRAVO" | strpos(municipality, "O BRAVO")>0
replace uniqueid=28034 if municipality =="SAN CARLOS"
replace uniqueid=28035 if municipality =="SAN FERNANDO"
replace uniqueid=28036 if municipality =="SAN NICOLAS"
replace uniqueid=28037 if municipality =="SOTO LA MARINA"
...
import excel Ayu_Seccion_2004_No_LN.xlsx, clear firstrow case(lower)
...
g missing = listanominal==. | municipality=="Mainero"

collapse (sum) missing listanominal pan - total , by (municipality section)
...
import excel Ayu_Seccion_2013.xlsx, clear firstrow case(lower)
...
foreach var in PRI PRD PT PCP PVEM PANAL PD PSD PAS PSN PartCardenista CDPPN PUP PEC PFC {
gen `var'_winner =0
...
saveold Tamaulipas_ALL.dta, replace version(12)
