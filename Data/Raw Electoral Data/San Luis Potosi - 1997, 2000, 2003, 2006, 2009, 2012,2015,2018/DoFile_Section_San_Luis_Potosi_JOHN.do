capture cd "D:\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\San Luis Potosi - 1997, 2000, 2003, 2006, 2009, 2012"
...
rename pps PPS
* Partido Demacrata Mexicano
rename pdm PD
* Nava Partido Politico
rename npp NPP
...
egen valid = rowtotal(PAN PRI PRD PartCardenista PT PVEM PPS PD NPP)

foreach var in PAN PRI PRD PartCardenista PT PVEM PPS PD NPP total valid{
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PartCardenista inv_mun_PT inv_mun_PVEM inv_mun_PPS inv_mun_PD inv_mun_NPP, gen(PAN_r PRI_r PRD_r PartCardenista_r PT_r PVEM_r PPS_r PD_r NPP_r)
drop inv_mun_*
...
replace winner = "NPP" if NPP_r==1
drop *_r


gen year = 1997
gen month ="July"
* use second round for runoff elections
replace month = "August" if municipality =="ALAQUINES" | municipality =="AXTLA DE TERRAZAS" | municipality =="CARDENAS" | ///
	municipality =="CATORCE" | municipality =="CEDRAL" | municipality =="CIUDAD DEL MAIZ" | municipality =="CIUDAD VALLES" | municipality =="CHARCAS" | ///
	municipality =="MATEHUALA" | municipality =="RAYON" | municipality =="RIOVERDE" | municipality =="SAN MARTIN CHALCHICUAUTLA" | ///
	municipality =="SAN NICOLAS TOLENTINO" | municipality =="SANTO DOMINGO" | municipality =="TAMASOPO" | municipality =="TAMPAMOLON CORONA" | ///
	municipality =="TAMUIN" | municipality =="TANQUIAN DE ESCOBEDO" | municipality =="VENEDO" | municipality =="VILLA DE ARISTA" | ///
	municipality =="VILLA DE GUADALUPE" | municipality =="VILLA DE RAMOS" | municipality =="ZARAGOZA"
replace municipality = municipality + " RUNOFF" if month=="August"

g runoff = month=="August"
...
import excel "Runoffs 1997.xlsx", sheet("Sheet1") cellrange(A4:O1019) firstrow clear

rename B section
drop if section==""
destring _all, replace

g municipality = DES_PUEBLO + " RUNOFF"
rename NAVA NPP

egen total = rowtotal(PAN-NULOS)
egen valid = rowtotal(PAN-NPP)

collapse (sum) PAN-NPP valid total, by (municipality section)
...
keep if ed==24 & month==7 & year==1997
rename lista listanominal
rename seccion section
...
foreach var in PAN PRI PRD PFCRN PT PVEM PPS PD NPP total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PFCRN inv_mun_PT inv_mun_PVEM inv_mun_PPS inv_mun_PD inv_mun_NPP, gen(PAN_r PRI_r PRD_r PFCRN_r PT_r PVEM_r PPS_r PD_r NPP_r)
drop inv_mun_*
...
replace winner = "PT" if PT_r==1
replace winner = "PVEM" if PVEM_r==1
replace winner = "PFCRN" if PFCRN_r==1
replace winner = "PPS" if PPS_r==1
replace winner = "PD" if PD_r==1
replace winner = "NPP" if NPP_r==1
drop *_r
...
save San_Luis_Potosi_Section_1997_runoff.dta,replace
...
replace winner = "PVEM_PCP" if PVEM_PCP_r==1
replace winner = "PRD_PT_PCD_PSN" if PRD_PT_PCD_PSN_r==1
...
gen year = 2000
gen month ="July"
replace month = "August" if municipality=="AQUISMON" | municipality=="CERRO DE SAN PEDRO" | municipality=="CIUDAD VALLES" | municipality=="EBANO" | municipality=="MATEHUALA" | municipality=="RAYON" | municipality=="RIOVERDE" | ///
	municipality=="SAN ANTONIO" | municipality=="SAN LUIS POTOSI" | municipality=="SANTO DOMINGO" | municipality=="TAMASOPO" | municipality=="TANLAJAS" | municipality=="TIERRA NUEVA" | municipality=="VENADO" | ///
	municipality=="VILLA DE ARRIAGA" | municipality=="VILLA DE RAMOS" | municipality=="ZARAGOZA"
replace municipality = municipality + " RUNOFF" if month=="August"
...
replace winner = "PRI_PVEM_PCP" if PRI_PVEM_PCP_r==1
replace winner = "PCP_PC" if PCP_PC_r==1
replace winner = "PCP_PAS_PC" if PCP_PAS_PC_r==1
replace winner = "PRD_PT" if PRD_PT_r==1
replace winner = "PRD_PT_PC" if PRD_PT_PC_r==1
replace winner = "PRD_PT_PCP_PC" if PRD_PT_PCP_PC_r==1
replace winner = "PRD_PVEM_PC" if PRD_PVEM_PC_r==1
replace winner = "PRI_PCP" if PRI_PCP_r==1
replace winner = "PRI_PT" if PRI_PT_r==1
replace winner = "PRI_PT_PVEM_PAS" if PRI_PT_PVEM_PAS_r==1
replace winner = "PRI_PT_PVEM" if PRI_PT_PVEM_r==1
replace winner = "PT_PC" if PT_PC_r==1
replace winner = "PT_PCP" if PT_PCP_r==1
replace winner = "PT_PCP_PC" if PT_PCP_PC_r==1
replace winner = "PRD_PT_PSN_PC" if PRD_PT_PSN_PC_r==1
drop *_r
...
import excel "Runoffs 2003.xlsx", sheet("Sheet1") cellrange(A6:X902) firstrow clear
...
foreach x of varlist PAN-CC1 {
	replace `x' = "0" if `x'=="-"
}

destring _all, replace

rename POLITICA municipality
replace municipality = "MEXQUITIC DE CARMONA RUNOFF" if B==21
replace municipality = "SOLEDAD DE GRACIANO SANCHEZ RUNOFF" if B==35
replace municipality = "RIOVERDE RUNOFF" if B==24
replace municipality = "CIUDAD VALLES RUNOFF" if B==13
replace municipality = "XILITLA RUNOFF" if B==55

g PRI_PVEM_PCP = PCP + APT + cc1solo if municipality=="SOLEDAD DE GRACIANO SANCHEZ RUNOFF"
replace PCP = . if municipality=="SOLEDAD DE GRACIANO SANCHEZ RUNOFF"
replace APT = . if municipality=="SOLEDAD DE GRACIANO SANCHEZ RUNOFF"

g PRI_PVEM = PRI + PVEM + cc1solo if municipality=="RIOVERDE RUNOFF"
replace PRI = . if municipality=="RIOVERDE RUNOFF"
replace PVEM = . if municipality=="RIOVERDE RUNOFF"

g PRI_PVEM_PT_PAS = PRI + PVEM + PT + PAS + cc1solo if municipality=="CIUDAD VALLES RUNOFF"
replace PRI = . if municipality=="CIUDAD VALLES RUNOFF"
replace PT = . if municipality=="CIUDAD VALLES RUNOFF"
replace PVEM = . if municipality=="CIUDAD VALLES RUNOFF"
replace PAS = . if municipality=="CIUDAD VALLES RUNOFF"

egen valid = rowtotal(PAN PRI PRD PRI_PVEM*)

collapse (sum) PAN PRI PRD PRI_PVEM* valid total listanominal, by (municipality section)

g uniqueid= .
replace uniqueid=24013 if municipality =="CIUDAD VALLES RUNOFF"
replace uniqueid=24021 if municipality =="MEXQUITIC DE CARMONA RUNOFF"
replace uniqueid=24024 if municipality =="RIOVERDE RUNOFF"
replace uniqueid=24035 if municipality =="SOLEDAD DE GRACIANO SANCHEZ RUNOFF"
replace uniqueid=24054 if municipality =="XILITLA RUNOFF"

g turnout = total/listanominal

foreach var in PAN PRI PRD PRI_PVEM_PCP PRI_PVEM PRI_PVEM_PT_PAS total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
	g inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PRI_PVEM_PCP inv_mun_PRI_PVEM inv_mun_PRI_PVEM_PT_PAS, gen(PAN_r PRI_r PRD_r PRI_PVEM_PCP_r PRI_PVEM_r PRI_PVEM_PT_PAS_r)
drop inv_mun_*
...
replace winner = "PRI_PVEM_PCP" if PRI_PVEM_PCP_r==1
replace winner = "PRI_PVEM" if PRI_PVEM_r==1
replace winner = "PRI_PVEM_PT_PAS" if PRI_PVEM_PT_PAS_r==1
drop *_r
...
save San_Luis_Potosi_Section_2003_runoff.dta,replace
...
replace winner = "PASC" if PASC_r==1
replace winner = "PAN_PANAL" if PAN_PANAL_r==1
replace winner = "PAN_PCP_PANAL" if PAN_PCP_PANAL_r==1
replace winner = "PRI_PASC" if PRI_PASC_r==1
replace winner = "PRI_PCP_PASC" if PRI_PCP_PASC_r==1
replace winner = "PVEM_PCP" if PVEM_PCP_r==1
replace winner = "PVEM_PCP_PANAL" if PVEM_PCP_PANAL_r==1
replace winner = "PCP_PANAL" if PCP_PANAL_r==1
drop *_r
...
replace winner = "PAN_PANAL" if PAN_PANAL_r==1
replace winner = "PAN_PCP_PANAL" if PAN_PCP_PANAL_r==1  
replace winner = "PAN_PT_PANAL" if PAN_PT_PANAL_r==1  
replace winner = "PANAL_PSD" if PANAL_PSD_r==1  
replace winner = "PRD_PC" if PRD_PC_r==1  
replace winner = "PRD_PCP" if PRD_PCP_r==1  
replace winner = "PRD_PCP_PC" if PRD_PCP_PC_r==1  
replace winner = "PRD_PT" if PRD_PT_r==1  
replace winner = "PRD_PT_PCP" if PRD_PT_PCP_r==1  
replace winner = "PRD_PT_PCP_PSD" if PRD_PT_PCP_PSD_r==1  
replace winner = "PRI_PANAL" if PRI_PANAL_r==1  
replace winner = "PRI_PSD" if PRI_PSD_r==1  
replace winner = "PRI_PVEM" if PRI_PVEM_r==1  
replace winner = "PT_PC" if PT_PC_r==1  
replace winner = "PT_PCP" if PT_PCP_r==1  
replace winner = "PT_PCP_PC" if PT_PCP_PC_r==1  
replace winner = "PT_PSD" if PT_PSD_r==1  
replace winner = "PVEM_PCP" if PVEM_PCP_r==1  
replace winner = "PVEM_PSD" if PVEM_PSD_r==1  
drop *_r
...
bys uniqueid: egen mun_PAN_PRD_PT_PVEM_PCP_PC_PANAL = sum(PAN_PRD_PT_PVEM_PCP_PC_PANAL) 
gen inv_mun_long= 1/mun_PAN_PRD_PT_PVEM_PCP_PC_PANAL
...
replace winner = "PANAL" if PANAL_r==1
replace winner = "PAN_PANAL" if PAN_PANAL_r==1
replace winner = "PAN_PT_PC_PANAL" if PAN_PT_PC_PANAL_r==1
replace winner = "PAN_PRD_PT_PVEM_PCP_PC_PANAL" if long_r==1
replace winner = "PRI_PVEM" if PRI_PVEM_r==1  
replace winner = "PRI_PVEM_PCP" if PRI_PVEM_PCP_r==1  
replace winner = "PRI_PCP" if PRI_PCP_r==1
replace winner = "PRD_PT_PC" if PRD_PT_PC_r==1  
...
replace winner = "PT_PC" if PT_PC_r==1  
replace winner = "PT_PCP_PC" if PT_PCP_PC_r==1  
replace winner = "PCP_PC" if PCP_PC==1  
drop *_r
...
use San_Luis_Potosi_Section_1997.dta, clear
append using San_Luis_Potosi_Section_2000.dta
append using San_Luis_Potosi_Section_2003.dta
append using San_Luis_Potosi_Section_2003_runoff.dta
bysort uniqueid year : egen runoff2 = mean(runoff)
* Remove the non-runoff elections from 2003
...
erase San_Luis_Potosi_Section_2003_runoff.dta
...
saveold "..\San_Luis_Potosi_ALL.dta", replace version(12)
