capture cd "D:\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Morelos - 1997, 2000, 2003, 2006, 2009, 2012"
...
egen valid = rowtotal(PAN PRI PRD PartCardenista PT PVEM PCM PPS PDM)

foreach var in PAN PRI PRD PartCardenista PT PVEM PCM PPS PDM total valid{
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PartCardenista inv_mun_PT inv_mun_PVEM inv_mun_PCM inv_mun_PPS inv_mun_PDM, gen(PAN_r PRI_r PRD_r PartCardenista_r PT_r PVEM_r PCM_r PPS_r PDM_r)
drop inv_mun_*
...
collapse (sum)  missing listanominal pan-  nulos total, by (municipality section)
...
gen winner = "PAN" if PAN_r==1  
replace winner = "PRI" if PRI_r ==1
replace winner = "PRD_PC_PCD_PSN" if PRD_PC_PCD_PSN_r==1
replace winner = "PT" if PT_r==1
replace winner = "PVEM" if PVEM_r==1
replace winner = "PCM" if PCM_r==1
...
import excel "Extraordinario 2001.xlsx", sheet("Sheet1") firstrow clear

rename SECCIÓN section
rename SUMA total
rename LISTA listanominal

collapse (sum) listanominal PAN-PVEM total, by (municipality section)

g uniqueid = 17016

g turnout = total/listanominal

egen valid = rowtotal(PAN PRI PRD UDO PVEM)

foreach var in PAN PRI PRD UDO PVEM listanominal total valid {
	bys uniqueid: egen mun_`var'= sum(`var') 
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_UDO inv_mun_PVEM, gen(PAN_r PRI_r PRD_r UDO_r PVEM_r)
drop inv_mun_*

gen winner = "PAN" if PAN_r==1  
replace winner = "PRI" if PRI_r ==1
replace winner = "PRD" if PRD_r==1
replace winner = "UDO" if UDO_r==1
replace winner = "PVEM" if PVEM_r==1
drop *_r
...
save Morelos_Section_2001.dta, replace
...
replace winner = "PAS" if PAS_r==1  
replace winner = "PVEM_PANAL" if PVEM_PANAL_r==1
...
replace winner = "PAN_PSD" if PAN_PSD_r==1  
replace winner = "PRI_PC" if PRI_PC_r ==1
...
append using Morelos_Section_2001.dta
...
erase Morelos_Section_2001.dta
...
saveold Morelos_ALL.dta, replace version(12)
