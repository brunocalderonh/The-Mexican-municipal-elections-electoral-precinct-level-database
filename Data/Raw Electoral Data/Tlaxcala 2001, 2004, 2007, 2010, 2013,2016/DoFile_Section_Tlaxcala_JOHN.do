...
capture cd "D:\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Tlaxcala 2001, 2004, 2007, 2010, 2013"
capture cd "C:\Users\Horacio\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Tlaxcala 2001, 2004, 2007, 2010, 2013"
...
import excel Ayu_Seccion_2001_No_LN.xlsx, clear firstrow case(lower)
...
* PCDT stands for Partido Centro Democratico de Tlaxcala
rename pcdt PCDT
rename psn PSN
rename pas PAS
...
* replace winners based on re-run of election in feb. 2002 (no data available by precinct for these elections)
*replace winner = "PAN" if municipality=="IXTENCO"
*replace winner = "PAS" if municipality=="ZACATELCO"
...
drop if _merge==2
drop _merge
erase Ayu_Seccion_2004_LN.dta

g ed = 29
g seccion = section
capture merge 1:m ed seccion using "..\..\all_months_years.dta", keepusing(month year lista)
keep if month==10 & year==2004
drop if _merge==2
drop _merge ed seccion year month
...
drop turnout mun_turnout listanominal
...
foreach var in PAN PRI PRD PT PVEM PC PANAL PAC PS PAC_PS PAN_PAC PRD_PT PRI_PVEM PT_PAC total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
...
import excel "Resultados Extraordinaria 8 Diciembre 2013.xlsx", sheet("Sheet1") firstrow clear
...
collapse (sum) PAN - PS total valid, by(municipality uniqueid section)
...
foreach var in PAN PRI PRD PT PVEM MC PANAL PAC PS total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
	g inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PT inv_mun_PVEM inv_mun_MC inv_mun_PANAL inv_mun_PAC inv_mun_PS, gen(PAN_r PRI_r PRD_r PT_r PVEM_r MC_r PANAL_r PAC_r PS_r)
drop inv_mun_*
...
save Tlaxcala_Section_2013_Extraordinario.dta, replace

**************************************************************************
**************************************************************************
**************************************************************************

import excel "Resultados Extraordinaria 23 Febrero 2014.xlsx", sheet("Hoja1") cellrange(A6:K13) firstrow clear
...
g uniqueid = 29022 
g municipality = "ACUAMANALA DE MIGUEL HIDALGO EXTRAORDINARIO"

collapse (sum) PAN - MC total valid, by(municipality uniqueid section)

preserve
use "..\..\all_months_years.dta", clear
keep if ed==29 & month==1 & year==2014
rename seccion section
rename lista listanominal
save "merge.dta", replace
restore
...
foreach var in PAN PRI PRD PT MC total valid listanominal {
	bys uniqueid: egen mun_`var'= sum(`var') 
	g inv_mun_`var'= 1/mun_`var'
}
...
rowranks inv_mun_PAN inv_mun_PRI inv_mun_PRD inv_mun_PT inv_mun_MC, gen(PAN_r PRI_r PRD_r PT_r MC_r)
drop inv_mun_*
...
save Tlaxcala_Section_2014_Extraordinario.dta, replace
...
append using Tlaxcala_Section_2013_Extraordinario.dta
append using Tlaxcala_Section_2014_Extraordinario.dta
...
erase Tlaxcala_Section_2013_Extraordinario.dta
erase Tlaxcala_Section_2014_Extraordinario.dta
...
saveold Tlaxcala_ALL.dta, replace version(12)
