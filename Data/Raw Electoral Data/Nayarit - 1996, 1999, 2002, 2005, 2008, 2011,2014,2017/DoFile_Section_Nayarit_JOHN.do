...
capture cd "D:\Dropbox\Incumbency Advantage\Data Analysis\Raw Data\Precinct\Nayarit - 1996, 1999, 2002, 2005, 2008, 2011 - Missing 2014 (but PREP available; requested it)"
...
replace uniqueid=18003 if municipality =="AMATLAN DE CANAS"
...
replace winner = "PMEP" if PMEP_r==1
replace winner = "PARM" if PARM_r==1
drop *_r
...
drop if _merge==2
drop _merge
erase Nayarit_Section_1996_to_Merge_with_1999.dta
...
gen winner = "PAN" if PAN_r==1  
replace winner = "PRI" if PRI_r ==1
replace winner = "PRD_PT_PRS" if PRD_PT_PRS_r==1
replace winner = "PVEM" if PVEM_r==1
replace winner = "PC" if PC_r==1
drop *_r
...
replace winner = "PT" if PT_r==1
replace winner = "PAS" if PAS_r==1
replace winner = "PC_PRS" if PC_PRS_r==1
drop *_r
...
saveold Nayarit_ALL.dta, replace version(12)
