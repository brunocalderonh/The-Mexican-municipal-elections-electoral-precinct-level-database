...
finaldb <- read_csv("Processed Data/nayarit/nayarit_incumbent_manipulator.csv")
...
assign_incumbent_vote <- function(data) {
  ...
  for (I in 1:nrow(data)) {
    ...
    if (!is.na(data$incumbent_vote[I])) {
      ...
      data$final_incumbent[I] <- NA  # Ensure NA if no vote found
    }
  }
...
output_path <- file.path(output_dir, "nayarit_vote_calculator.csv")
