...
assign_incumbent_vote <- function(data) {
  ...
  data <- data %>%
    mutate(incumbent_vote = NA,
           party_component = NA,
           final_incumbent = NA)  # Track the actual source of incumbent_vote
    ...
    if (str_detect(incumbent_party, "_")) {
      ...
      for (var in candidate_vars) {
        if (!is.na(data[[var]][I]) && data[[var]][I] != 0) {
          data$incumbent_vote[I] <- data[[var]][I]
          data$party_component[I] <- var
          final_incumbent_value <- incumbent_party  # Track the single party as final_incumbent
          valid_found <- TRUE
          break
        }
      }
      ...
      if (!valid_found) {
        ...
        for (var in broader_coalition_vars) {
          if (!is.na(data[[var]][I]) && data[[var]][I] != 0) {
            data$incumbent_vote[I] <- data[[var]][I]
            data$party_component[I] <- var
            final_incumbent_value <- incumbent_party  # Track the broader coalition
            break
          }
        }
      }
