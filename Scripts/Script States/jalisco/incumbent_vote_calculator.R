...
finaldb <- read_csv("Processed Data/jalisco/jalisco_incumbent_manipulator.csv")
...
assign_incumbent_vote <- function(data) {
  ...
  for (I in 1:nrow(data)) {
    ...
    final_incumbent_value <- NA  # Initialize final_incumbent tracker
    ...
    if (str_detect(incumbent_party, "_")) {
      ...
      for (var in candidate_vars) {
        if (!is.na(data[[var]][I]) && data[[var]][I] != 0) {
          data$incumbent_vote[I] <- data[[var]][I]
          data$party_component[I] <- var
          final_incumbent_value <- party  # Track the standalone party
          valid_found <- TRUE
          break
        }
      }
...
output_path <- file.path(output_dir, "jalisco_vote_calculator.csv")
