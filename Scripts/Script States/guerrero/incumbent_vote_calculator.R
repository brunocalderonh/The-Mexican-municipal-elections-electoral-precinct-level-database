    ...
    if (str_detect(incumbent_party, "_")) {
      ...
      for (var in coalition_vars) {
        if (!is.na(data[[var]][I]) && data[[var]][I] != 0) {
          ...
          final_incumbent_value <- incumbent_party  # Ensure coalition is tracked as final_incumbent
          valid_found <- TRUE
          break
        }
      }
      ...
      for (var in candidate_vars) {
        if (!is.na(data[[var]][I]) && data[[var]][I] != 0) {
          data$incumbent_vote[I] <- data[[var]][I]
          data$party_component[I] <- var
          final_incumbent_value <- incumbent_party  # Ensure single party is tracked as final_incumbent
          valid_found <- TRUE
          break
        }
      }
