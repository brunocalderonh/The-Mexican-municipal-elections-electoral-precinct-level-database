...
finaldb <- read_csv("Processed Data/veracruz/veracruz_incumbent_manipulator.csv")
...
assign_incumbent_vote <- function(data) {
  ...
  for (I in 1:nrow(data)) {
    # Determine the incumbent party by prioritizing non-NA values
    ...
    if (str_detect(incumbent_party, "_")) {
      ...
      for (party in parties) {
        if (party %in% c(data$incumbent_party_magar[I], data$incumbent_party_Horacio[I], 
                         data$incumbent_party_inafed[I], data$incumbent_party_JL[I])) {
...
output_dir <- file.path(getwd(), "Processed Data/veracruz")
output_path <- file.path(output_dir, "veracruz_vote_calculator.csv")
