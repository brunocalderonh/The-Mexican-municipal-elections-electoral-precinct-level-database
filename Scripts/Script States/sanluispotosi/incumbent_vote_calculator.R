...
finaldb <- read_csv("Processed Data/sanluispotosi/sanluispotosi_incumbent_manipulator.csv")
...
assign_incumbent_vote <- function(data) {
  ...
  for (I in 1:nrow(data)) {
    ...
    final_incumbent_value <- NA  # Initialize the tracker for final_incumbent
    ...
    if (str_detect(incumbent_party, "_")) {
      ...
      for (var in coalition_vars) {
        if (!is.na(data[[var]][I]) && data[[var]][I] != 0) {
          ...
          final_incumbent_value <- incumbent_party  # Coalition uses the resolved value
          valid_found <- TRUE
          break
        }
      }
      ...
      # Check broader coalitions if no valid single party vote is found
      if (!valid_found) {
...
output_dir <- file.path(getwd(), "Processed Data/sanluispotosi")
output_path <- file.path(output_dir, "sanluispotosi_vote_calculator.csv")
