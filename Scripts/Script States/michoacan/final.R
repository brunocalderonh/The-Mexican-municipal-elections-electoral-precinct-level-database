...
db <- read_excel("michoacan/michoacan_collapsed_edited.xlsx")
og <- read.csv("michoacan/michoacan_FINAL_draft.csv")
...
validation <- db_subset %>%
  group_by(uniqueid, year, state_incumbent_candidate, state_year) %>%
  filter(is.na(researched_incumbent) & n() > 1) %>%
  ungroup()

db_subset <- db_subset %>% 
  anti_join(validation, by = c("uniqueid", "year", "state_incumbent_candidate", "state_year", "researched_incumbent", "source_researched_incumbent"))
...
write.csv(merged_data, "/Users/brunocalderon/Library/CloudStorage/OneDrive-Personal/Documents/ITAM/RA - Horacio/Monitoring Brokers/Data/States/michoacan/michoacan_FINALV.csv")
