...
finaldb <- read_csv("Processed Data/michoacan/michoacan_incumbent_manipulator.csv")
...
finaldb <- finaldb %>%
  mutate(incumbent_party_magar = ifelse(year == 2001 & incumbent_party_magar == "PFCRN", "PartCardenista", incumbent_party_magar)) %>%
  mutate(incumbent_party_magar = ifelse(year == 1998 & incumbent_party_magar == "PFCRN", "PartCardenista", incumbent_party_magar)) %>% 
mutate(runnerup_party_magar = ifelse(year == 2001 & runnerup_party_magar == "PFCRN", "PartCardenista", runnerup_party_magar)) %>%
  mutate(runnerup_party_magar = ifelse(year == 1998 & runnerup_party_magar == "PFCRN", "PartCardenista", runnerup_party_magar))
...
assign_incumbent_vote <- function(data) {
  ...
  for (I in 1:nrow(data)) {
    ...
    # Determine the incumbent party, checking each source in priority order
    incumbent_party <- data$incumbent_party_magar[I]
    if (is.na(incumbent_party) || incumbent_party == "") {
      incumbent_party <- data$incumbent_party_Horacio[I]
    }
    if (is.na(incumbent_party) || incumbent_party == "") {
    ...
    # Skip if incumbent_party is still NA or empty
    ...
    if (str_detect(incumbent_party, "_")) {
      ...
      # Proceed with coalition logic if no individual party from the coalition is found
      ...
      for (party_var in party_vars) {
        ...
        if (!str_detect(party_var, "\\bPANAL\\b") && str_detect(party_var, "\\bPAN\\b") || !str_detect(party_var, "\\bPAN\\b")) {
          if (!is.na(data[[party_var]][I]) && data[[party_var]][I] != 0) {
            data$incumbent_vote[I] <- data[[party_var]][I]
            data$party_component[I] <- party_var
            break
...
output_path <- file.path(output_dir, "michoacan_vote_calculator.csv")
