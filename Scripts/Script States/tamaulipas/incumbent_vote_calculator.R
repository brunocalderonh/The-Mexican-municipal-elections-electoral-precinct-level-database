...
finaldb <- read_csv("Processed Data/tamaulipas/tamaulipas_incumbent_manipulator.csv")
  ...
  for (I in 1:nrow(data)) {
    ...
    if (is.na(incumbent_party) || incumbent_party == "") {
      ...
      if (length(all_incumbent_parties) > 0) {
        ...
        final_incumbent_value <- all_incumbent_parties[1]  # Use the first valid party
      } else {
      ...
      for (coalition in coalition_parties) {
        ...
        for (single_party in single_parties) {
          if (single_party %in% coalition_components) {
            ...
            final_incumbent_value <- single_party  # Use the single party
            break
          }
        }
...
output_path <- file.path(output_dir, "tamaulipas_vote_calculator.csv")
