...
db <- read_excel("Data/collapsed database manual cases/campeche_collapsed_edited.xlsx")
og <- read.csv("Processed Data/campeche/campeche_vote_calculator.csv")
...
correct_runnerup_vote <- function(data) {
  ...
  for (I in 1:nrow(data)) {
    ...
    if (str_detect(runnerup_party, "_")) {
      ...
      exact_match_vars <- coalition_vars[sapply(coalition_vars, function(x) {
        party_components <- unlist(str_split(x, "_"))
...
output_path <- file.path(output_dir, "campeche_final.csv")
