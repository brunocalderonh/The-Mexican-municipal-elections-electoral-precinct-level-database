...
finaldb <- read_csv("Processed Data/mexico/mexico_incumbent_manipulator.csv")
...
replace1 <- function(party_str) {
  if (!is.na(party_str)) {
    if (party_str == "PRD_PT_MC") {
      party_str <- "PRD_PT_PC"
    } else if (party_str == "PT_MC") {
      party_str <- "PT_PC"
    } else if (party_str == "PRD_MC") {
      party_str <- "PRD_PC"
    }
  }
...
assign_incumbent_vote <- function(data) {
  ...
  data <- data %>%
    mutate(incumbent_vote = NA,
           party_component = NA,
           final_incumbent = NA)  # New column to track the final incumbent party or coalition
...
output_path <- file.path(output_dir, "mexico_vote_calculator.csv")
