...
db <- read_excel("cdmx/cdmx_collapsed_edited.xlsx")
og <- read.csv("cdmx/cdmx_FINAL_draft.csv")
...
update_missing_votes <- function(data) {
  
  # List of columns to check for incumbent parties
  incumbent_cols <- c("incumbent_party_magar", "incumbent_party_JL", "incumbent_party_inafed", "incumbent_party_Horacio", "researched_incumbent")
  ...
  for (I in which(is.na(data$incumbent_vote))) {
    ...
    # Check the incumbent columns for the party
    for (col in incumbent_cols) {
      if (!is.na(data[[col]][I]) && data[[col]][I] != "") {
        incumbent_party <- data[[col]][I]
        break
      }
    }
    
    # If an incumbent party is found
...
merged_data <- update_missing_votes(merged_data)
...
write.csv(merged_data, "/Users/brunocalderon/Library/CloudStorage/OneDrive-Personal/Documents/ITAM/RA - Horacio/Monitoring Brokers/Data/States/cdmx/cdmx_FINALV.csv")
