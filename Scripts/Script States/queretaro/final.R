...
db <- read_excel("Data/collapsed database manual cases/queretaro_collapsed_edited.xlsx")
og <- read.csv("Processed Data/queretaro/queretaro_vote_calculator.csv")
...
focused1 <- function(data) {
  
  # Filter for the target observations
  target_data <- data %>%
  ...
  for (I in 1:nrow(target_data)) {
    ...
    # Regex to match PAN specifically (exclude PANAL)
    party_regex <- "(^PAN$|_PAN$|^PAN_)"
    
    # Find columns matching the target party
    candidate_vars <- names(target_data)[grepl(party_regex, names(target_data))]
  ...
  data <- data %>%
    rows_update(target_data, by = c("uniqueid", "year", "section"))
