...
# Load the data
finaldb <- read_dta( "/Users/brunocalderon/Library/CloudStorage/OneDrive-Personal/Documents/ITAM/RA - Horacio/Monitoring Brokers/Data/States/queretaro/queretaro_merged_IncumbentVote.dta")
...
assign_incumbent_vote <- function(data) {
  ...
  for (I in 1:nrow(data)) {
    ...
    if (str_detect(incumbent_party, "_")) {
      ...
      unique_parties_in_others <- unique(c(data$incumbent_party_JL[I], 
                                           data$incumbent_party_Horacio[I], 
                                           data$incumbent_party_inafed[I]))
      
      if (length(unique_parties_in_others) == 1 && !is.na(unique_parties_in_others) && unique_parties_in_others != "") {
        # If all other incumbent_party variables have the same individual party, use that
        individual_party <- unique_parties_in_others
      ...
      if (individual_party_found && is.na(data$incumbent_vote[I])) {
        if (length(unique_parties_in_others) == 1 && !is.na(unique_parties_in_others) && unique_parties_in_others != "") {
          # Handle the case where all other incumbent_party variables have the same individual party
          party_vars <- names(data)[str_detect(names(data), paste0("^", unique_parties_in_others, "$"))]
...
write.csv(finaldb, "/Users/brunocalderon/Library/CloudStorage/OneDrive-Personal/Documents/ITAM/RA - Horacio/Monitoring Brokers/Data/States/queretaro/queretaro_FINAL_draft.csv")



#CLEAN DB
  # Select only the desired columns
  queretaro_finaldb <- finaldb %>% 
  select(
    state,
    mun,
    section,
  ...
  write.csv(queretaro_finaldb, "/Users/brunocalderon/Library/CloudStorage/OneDrive-Personal/Documents/ITAM/RA - Horacio/Monitoring Brokers/Data/States/queretaro/queretaro_FINAL.csv")
